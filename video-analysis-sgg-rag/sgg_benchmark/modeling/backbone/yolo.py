...
# Try to import ultralytics utilities, with fallbacks for different versions
try:
...
try:
    ...
    feature_visualization = None

from pathlib import Path

# Try to import torch_safe_load, if not available use alternative
try:
    from ultralytics.nn.tasks import torch_safe_load
except ImportError:
    # Fallback for newer ultralytics versions
    def torch_safe_load(file):
        """Load a torch model with additional safety checks"""
        try:
...
class YoloModel(DetectionModel):
    ...
    def forward(self, x, profile=False, visualize=False, embed=None):
        ...
        for i, m in enumerate(self.model):
            ...
            """
            if visualize and feature_visualization is not None:
    ...
    def load(self, weights_path: str, task=None):
        ...
        weights = torch_safe_load(weights_path)
    ...
    def postprocess(self, preds, image_sizes):
        ...
        for i, (pred, idx) in enumerate(zip(preds, indices)):
            ...
            if ops is not None:
                ...
                # Fallback: simple rescaling if ops is not available
                scale_x = out_img_size[0] / self.input_size
                scale_y = out_img_size[1] / self.input_size
                boxes[:, [0, 2]] *= scale_x
                boxes[:, [1, 3]] *= scale_y
