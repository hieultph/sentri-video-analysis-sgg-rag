...
def align_and_update_state_dicts(model_state_dict, loaded_state_dict, load_mapping, verbose=False):
    ...
    for i, key in enumerate(mapped_current_keys):
        for source_key, target_key in load_mapping.items():
            if source_key in key:
                ...
                logger.debug("MAPPING {} in current model to {} in loaded model.".format(key, mapped_current_keys[i]))
    ...
    for idx_new, idx_old in enumerate(idxs.tolist()):
        if idx_old == -1:
            ...
            logger.debug("NO-MATCHING of current module: {} of shape {}".format(key, 
