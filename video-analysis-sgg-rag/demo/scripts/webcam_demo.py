...
def main(args):
    ...
    # Open the webcam
    cap = cv2.VideoCapture(0)  # Try index 0 first (most common)
    ...
    # If still not working, try index -1
    if not cap.isOpened():
        print("Camera index 1 failed, trying index -1...")
        cap = cv2.VideoCapture(-1)
    
    # Check if any camera worked
    if not cap.isOpened():
    ...
    if visu_type == 'image':
        ...
        cv2.resizeWindow('Graph', min(800, window_width), min(600, window_height))
    ...
    while True:
        ...
        # Optional: Resize the image for display (uncomment to use)
        # display_width, display_height = 1280, 720  # Adjust these values as needed
        # img = cv2.resize(img, (display_width, display_height))
        
        if visu_type == 'image':
            graph = cv2.cvtColor(graph, cv2.COLOR_BGR2RGB)
            # Optional: Resize the graph for display (uncomment to use)
            # graph = cv2.resize(graph, (800, 600))
